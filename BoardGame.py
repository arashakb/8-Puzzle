...
class Board:
    ...
    def goal_test(self):
        if np.array_equal(self.state, np.array([1, 2, 3, 4, 5, 6, 7, 8, 0])):
            return True
        else:
            return False
    ...
    def manhattan(self):
        ...
        goal = self.index(np.array([1, 2, 3, 4, 5, 6, 7, 8, 0]))
        return sum((abs(state // 3 - goal // 3) + abs(state % 3 - goal % 3))[1:])

    @staticmethod
    def index(state):
        index = np.array(np.array([1, 2, 3, 4, 5, 6, 7, 8, 0]))
    ...
    def up(self):
        if self.zero > 2:
            return Board(self.swap(self.zero, self.zero - 3), 
            self, 'Up', self.depth + 1)
        else:
            return None
    ...
    def down(self):
        if self.zero < 6:
            return Board(self.swap(self.zero, self.zero + 3), 
            self, 'Down', self.depth + 1)
        else:
            return None

    def left(self):
        if self.zero % 3 != 0:
            return Board(self.swap(self.zero, self.zero - 1), 
    ...
    def right(self):
        if (self.zero + 1) % 3 != 0:
            return Board(self.swap(self.zero, self.zero + 1), 
            self, 'Right', self.depth + 1)
        else:
            return None

    def neighbors(self):
        neighbors = [self.up(), self.down(), 
